...
{
	if ((ft_strlen_gnl(*buff) == 0 && ft_strlen_gnl(*send) == 0) && (fd >= 0))
	{
		free(*send);
		send = NULL;
		return (NULL);
...
char	*get_next_line(int fd)
{
	...
	if (find_new_line(buff) != -1)
	{
		...
		buff = ft_substr(buff, find_new_line(buff) + 1,
				ft_strlen_gnl(buff) - find_new_line(buff), 1);
	}
	else
	{
		send = ft_substr(buff, 0, ft_strlen_gnl(buff), 0);
		free(buff);
		buff = NULL;
	}
